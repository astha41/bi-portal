...
export default function PerformanceDashboard({ user, role }) {
  ...
  // initialize favorite state and subscribe to updates
  useEffect(() => {
  ...
  // set mount height so embed fills viewport area and embed dashboard
  const updateMountHeight = useCallback(() => {
    try {
      ...
      // ensure injected iframe/elements occupy full size
  ...
  useEffect(() => {
    ...
    // Use MutationObserver to ensure any injected iframe fills the container
  ...
  // Inline styles for large dashboard layout
  const styles = {
    page: {
      padding: 20,
      display: "flex",
